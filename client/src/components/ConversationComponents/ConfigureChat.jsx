...
export const ConfigureChat = ({inspectedConversation = [], setConfigureChat, setInspectedConversation}) => {

    const [showGroupUsers, setShowGroupUsers] = useState(false)
    const [addNewGroupMember, setAddNewGroupMember] = useState(false);
    ...
    return (
        <motion.main
            ...
            <svg
                onClick={() => {
                    if (addNewGroupMember) {
                        setAddNewGroupMember(false);
                    } else {
                        setConfigureChat(false);
                    }
                }}
            ...
            <AnimatePresence mode="wait">
                {addNewGroupMember ? (
                    <AddNewGroupMember
                        groupMembers={inspectedConversation}
                        setInspectedConversation={setInspectedConversation}
                        setAddNewGroupMember={setAddNewGroupMember}
                        key="add-member" />
                ) : (
                    <motion.div key="main-config">
                        ...
                        <GroupAvatar
                            group={inspectedConversation}
                            size={50}
                        />

                        <h1>{inspectedConversation.name}</h1>
                        ...
                        >
                            <BottomSheetItem
                                ...
                                admin={inspectedConversation?.admin_id || ''}
                                ...
                                groupMembers={inspectedConversation.GroupMembers || []}
                            ...
                            <BottomSheetItem
                                showDropDown={() => setAddNewGroupMember(true)}
